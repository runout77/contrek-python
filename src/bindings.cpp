...
OptionValue pyobj_to_optionvalue(const py::handle& obj) {
  ...
  throw std::invalid_argument("Unsupported Options value type; use bool, int, float, str, "
...
py::object optionvalue_to_pyobj(const OptionValue& v) {
  ...
  if (v.is_double())
    return py::float_(v.as_double());
  if (v.is_string())
    return py::str(v.as_string());
  if (v.is_identifier())
    return py::str(v.as_identifier().value);
  if (v.is_options())
    return options_to_pydict(v.as_options());
  return py::none();
}
...
template <typename PointSeq> py::array_t<int32_t> points_to_ndarray(const PointSeq& pts) {
...
public:
  PyRasterStreamer(RasterSource& source, uint32_t stripe_height) : source_(source), streamer_(source, stripe_height) {}
  ...
  uint32_t stripe_height() const { return streamer_.stripe_height(); }
...
PYBIND11_MODULE(_contrek, m) {
  ...
  m.def(
      ...
      py::arg("image_path"), py::arg("config") = Contrek::Config{},
      R"doc(
  ...
  m.def("opencv_contour_to_cell_boundary", &opencv_contour_to_cell_boundary, py::arg("points"), py::arg("bounds"));
  ...
  py::class_<FastPngBitmap, Bitmap>(m, "FastPngBitmap")
      .def(py::init<std::string>(), py::arg("path"), "Load and decode a PNG file from disk.");
  py::class_<RawBitmap, Bitmap>(m, "RawBitmap", py::buffer_protocol())
      ...
      .def_buffer([](RawBitmap& bitmap) -> py::buffer_info {
        ...
        return py::buffer_info(
            const_cast<unsigned char*>(bitmap.get_row_ptr(0)), sizeof(unsigned char),
            py::format_descriptor<unsigned char>::format(), 3,
            {static_cast<py::ssize_t>(height), static_cast<py::ssize_t>(width), static_cast<py::ssize_t>(bpp)},
            {static_cast<py::ssize_t>(width * bpp), static_cast<py::ssize_t>(bpp), static_cast<py::ssize_t>(1)});
      });
  m.def(
      "find_polygons",
  ...
  py::class_<TiffSource, RasterSource>(m, "TiffSource")
      .def(py::init<const std::string&, bool>(), py::arg("path"), py::arg("suppress_warnings") = false)
  ...
  py::class_<PyRasterStreamer>(m, "RasterStreamer")
      .def(py::init<RasterSource&, uint32_t>(), py::arg("source"), py::arg("stripe_height"), py::keep_alive<1, 2>())
      .def_property_readonly("stripe_height", &PyRasterStreamer::stripe_height)
      .def("each", &PyRasterStreamer::each, py::arg("bitmap"), py::arg("callback"));
  ...
  m.def("make_result_from_polygons", &make_result_from_polygons, py::arg("polygons"), py::arg("width"),
        py::arg("height"), py::arg("versus") = Contrek::Versus::A,
        R"doc(
  ...
  py::class_<PySvgStreamingMerger>(m, "SvgStreamingMerger")
      .def(py::init<py::dict, std::string, int, int>(), py::arg("options"), py::arg("output_path"), py::arg("width"),
           py::arg("height"), "Streams polygons to an SVG file incrementally as tiles are added.")
      .def("add_tile", &PySvgStreamingMerger::add_tile, py::arg("tile"), py::arg("flush") = false,
  ...
  py::class_<PyGeoJsonStreamingMerger>(m, "GeoJsonStreamingMerger")
      .def(py::init<py::dict, std::string, uint32_t>(), py::arg("options"), py::arg("output_path"),
           py::arg("pixel_value"), "Streams polygons to a GeoJSON file incrementally as tiles are added.")
      .def("add_tile", &PyGeoJsonStreamingMerger::add_tile, py::arg("tile"), py::arg("flush") = false,
           "Feed one tile's RawProcessResult. Pass flush=True on the last tile.")
      .def("process_info", &PyGeoJsonStreamingMerger::process_info,
           "Finalize the stream and return summary metadata as a dict.");

  py::class_<PyVerticalMerger>(m, "VerticalMerger")
      .def(py::init<int, py::dict>(), py::arg("number_of_threads") = 0, py::arg("options") = py::dict(),
           "In-memory tile merge (no file output) -- combines RawProcessResult "
           "tiles (e.g. from find_polygons_raw()) into one merged result.")
      .def("add_tile", &PyVerticalMerger::add_tile, py::arg("tile"), "Feed one tile's RawProcessResult into the merge.")
