...
def test_svg_streaming_merger(sample_stripes):
    ...
    try:
        ...
        assert (
            filecmp.cmp(
                str(stream_fixture_path("test_16x23_w4.svg")), temp_path, shallow=False
            )
            == True
        )
...
def test_geojson_streaming_merger(sample_stripes):
    ...
    try:
        merger = contrek.GeoJsonStreamingMerger(
            options={"bounds": True}, output_path=temp_path, pixel_value=34
        )
        for i, stripe in enumerate(sample_stripes):
            bitmap = contrek.Bitmap(stripe, width)
            tile = contrek.find_polygons_raw(
        ...
        assert (
            filecmp.cmp(
                str(stream_fixture_path("test_16x23_w4.geojson")),
                temp_path,
                shallow=False,
            )
            == True
...
def test_geojson_streaming_merger_with_compression(triangle_stripes):
    ...
    try:
        merger = contrek.GeoJsonStreamingMerger(
            options={
                "bounds": True,
                "compress": {"uniq": True, "linear": True, "douglas_peucker": True},
            },
            output_path=temp_path,
            pixel_value=34,
        )
        ...
        assert (
            filecmp.cmp(
                str(stream_fixture_path("test_18x11_w2.geojson")),
                temp_path,
                shallow=False,
            )
            == True
...
def test_svg_streaming_merger_with_compression(triangle_stripes):
    ...
    try:
        ...
        assert (
            filecmp.cmp(
                str(stream_fixture_path("test_18x11_w2.svg")), temp_path, shallow=False
            )
            == True
        )
...
def test_vertical_merger():
    ...
    expected = load_expected_polygons(
        fixture_path(
            "concurrent", "merging", "merge_mode_example_14_vertical_bug_fixing.json"
        )
    )
...
def test_horizontal_merger():
    ...
    expected = load_expected_polygons(
        fixture_path("concurrent", "merging", "merge_mode.json")
    )
    assert_polygons_match(result["polygons"], expected)


def test_exceptions():
    left = "00000000000         0         0000000000"
    right = "0000000000         0         00000000000"
    right_higher = "0000000000         0         0         00000000000"
    ...
    with pytest.raises(
        ValueError, match="All results must have the same versus option"
    ):
        merger.add_tile(result_right)
    ...
    with pytest.raises(
        ValueError, match="Result with not supported postprocessing compression mode"
    ):
...
def test_merging_existings_coordinates():
    ...
    result_up = contrek.make_result_from_polygons(
        polygons_up, width=12, height=5, versus=contrek.Versus.ANTICLOCKWISE
    )
    polygons_down = [
        {
    ...
    result_down = contrek.make_result_from_polygons(
        polygons_down, width=12, height=5, versus=contrek.Versus.ANTICLOCKWISE
    )
    ...
    assert result["height"] == 9
    expected = load_expected_polygons(
        fixture_path("concurrent", "merging", "merge_mode_from_existing_polygons.json")
    )
    assert_polygons_match(result["polygons"], expected)
